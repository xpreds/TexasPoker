...
public class Game extends JComponent {
  ...
  public void showdown() {
    ...
    Çözülmemiş errorlar:
    *** blind göstergesi
    *** para pot a direkt eklensin tur sonu değil
    *** high card durumu
    *** oyunun sonunda paranın winnera eklenememesi ve oyunun saçmalaması
*/

for(int j = 0; j < playerList.size();j++){

    ArrayList<int[]> allHands = com.getArrayList(playerList.get(j).getCard(), table);
    ...
    int[][] data = new int[4][4];// bu arrayin ilk dimensionunda winningHandleri, ikincisinde ise player indexleri sakl?yacaz
    data = {{playerList.get(0).getWinningHand(),0},{playerList.get(1).getWinningHand(),1},{playerList.get(2).getWinningHand(),2},{playerList.get(3).getWinningHand(),3}};
Arrays.sort(data, new Comparator<int[]>() {
    ...
    public int compare(int[] i1, int[] i2) {
     ...
     int winner = data[0][0]; //burdan sonra nas?l yapar?m bilmiyorum ama data n?n bu eleman? winner olmas? laz?m
   ...
   /*   ArrayList<Integer> val = new ArrayList<Integer>();
    ...
    for(int j = 0; j < playerList.size(); j++) {
      ...
      playerList.get(i).setWinningHand(val.get(i));
    }
    winner = 5;
